...
### 2.1 控制面：`sessions_send` (sessionKey Routing)

用于：派单、ACK、催办、简短结论、正式控制面消息。

**关键规则：sessionKey > label**
- `sessionKey` 是 canonical live session addressing 方式
- `label` 仅作为兜底/辅助路由，不能依赖
- 内部控制面通信必须通过 `sessionKey` 进行精确寻址

```
控制面寻址优先级：
1. sessionKey (精确到具体会话，首选)
2. label (模糊匹配，仅兜底)
```
...
示例：
- agent:main:completion-relay
- agent:trading:daily-analysis
- agent:macro:market-outlook
```
...
## 2.5 控制面 vs 消息面 (Control Plane vs Messaging Plane)

这是两个**完全独立**的平面，不能混用。

### 控制面 (Control Plane)
- **工具**: `sessions_send` with `sessionKey`
- **用途**: Agent-to-agent 控制消息、派单、ACK、终态通知
- **寻址**: 基于 `sessionKey`（精确会话级寻址）
- **可靠性**: 有 ACK 机制、状态追踪、超时处理

### 消息面 (Messaging Plane)
- **工具**: `message.send` / provider channel (Discord/Slack等)
- **用途**: **仅用于用户可见的消息投递**
- **寻址**: 基于 channel/label（广播或用户可见频道）
- **可靠性**: 无内置 ACK，不保证被处理
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   控制面 (Control Plane)           消息面 (Messaging Plane)      │
│   ──────────────────────           ────────────────────          │
│                                                                 │
│   sessions_send()                  message.send()                │
│   ├── sessionKey (精确)             ├── channel/label            │
│   ├── 内部 Agent 通信               ├── 用户可见消息             │
│   ├── 有 ACK 机制                   ├── 无 ACK 机制               │
│   └── 用于: 派单/控制/状态          └── 用于: 用户通知/展示       │
│                                                                 │
│   ⚠️ 禁止用 message.send 发送控制消息！                         │
│   ⚠️ 禁止用控制面发送用户可见内容！                             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 典型故障模式：Discord allowBots=mentions

当 Provider 配置为 `allowBots = mentions` 时：

```
Bot A 发送非 mention 消息到共享频道
         ↓
消息到达频道 (message delivered)
         ↓
Bot B 的配置: requireMention=true
         ↓
Bot B 不处理该消息 (不是 mention)
         ↓
❌ 控制请求未被接收 (control request NOT received)
```
...
- 消息投递成功（消息面）只表示消息到达频道
- 控制请求被处理（控制面）需要接收方显式 ACK
- 用 `sessions_send` 的 `sessionKey` 确保精确投递和处理追踪
